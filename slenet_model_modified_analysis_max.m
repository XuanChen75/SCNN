function []=slenet_model_modified_analysis_max()
...
analysis_max_0b=zeros(300,60000,10);
cnn=zeros(60000,10);
...
for n=1:1:60000
            ...
            for ii=1:1:10
                ...
                for cc=1:1:6
                    for yy=1:1:5
						...
						row=2*(ii+xx-1);
						col=2*(jj+yy-1);
						input2(counti,countj)=max(cnn1((col-2)*28+row,cc),cnn1((col-2)*28+row-1,cc));
                        input2(counti,countj)=max(input2(counti,countj),cnn1((col-1)*28+row-1,cc));
                        input2(counti,countj)=max(input2(counti,countj),cnn1((col-1)*28+row,cc));
                        end
                    end
                end
            end
        ...
        countj=0;
                for cc=1:1:16
                    for xx=1:1:5
						...
						row=2*(xx);
                        col=2*(yy);
                        input3(1,countj)=max(cnn2((col-2)*10+row,cc),cnn2((col-2)*10+row-1,cc));
                        input3(1,countj)=max(input3(1,countj),cnn2((col-1)*10+row-1,cc));
                        input3(1,countj)=max(input3(1,countj),cnn2((col-1)*10+row,cc));
                        end
                    end
                end
    ...
    for time=1:1:300
		...
		for ii=1:1:10
			analysis_max_0b(time,n,ii)=count_spike(ii);
			cnn(n,ii)=cnn5(1,ii);
		end
		fprintf('%d %d\n',n,time);
    end
...
save analysis_max_0b.mat analysis_max_0b cnn
end